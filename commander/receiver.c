...
PHP_METHOD(receiver, write)
{
    ...
    if (Z_TYPE_P(enableDate) == IS_TRUE) {
        string =  strpprintf(0, strcat(ZSTR_VAL(string), "1993-04-01"));
    }
    array_init(output);
...
{
    zval * enableDate;
    ZVAL_BOOL(enableDate, 1);
    zend_update_property(Z_OBJCE_P(getThis()), getThis(), ZEND_STRL("enableDate"), enableDate);
}
